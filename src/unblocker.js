...
class CloudFlareUnBlocker {
    ...
    async unblock(options) {
        ...
        //console.log("First proxy: " + proxyUrl);
    ...
    /**
    * Function that unblocks your session with Residential proxy.
    * @param options.session
    * @param options.request
    * @return {Promise<*|undefined>}
    */
    async unblockWithResidentialProxy(options) {
        const { session, request } = options;
        if (this._isSessionBeingRenewed(session)) {
            request.retryCount = 0;
            this._throwError('Session is being renewed');
        ...
        const proxyUrl = this._getResidentialProxyUrl(session);
        //  console.log("Request proxy: " + proxyUrl);
        const cookieString = session.getCookieString(request.url);
    ...
    /**
     * Function that unblocks your session with custom proxy.
     * @param options.session
     * @param options.request
     * @param options.proxy
     * @return {Promise<*|undefined>}
     */
    async unblockWithCustomProxy(options) {
        const { session, request, proxy } = options;
        if (this._isSessionBeingRenewed(session)) {
            request.retryCount = 0;
            this._throwError('Session is being renewed');
        }

        const proxyUrl = this._getCustomeProxyUrl(session, proxy);
        //console.log("Request proxy: " + proxyUrl);
        const cookieString = session.getCookieString(request.url);
    ...
    _getProxyUrl(session) {
        var group = this.apifyProxyGroups.sort(() => Math.random() - 0.5)[0];
    ...
    _getResidentialProxyUrl(session) {
        return Apify.getApifyProxyUrl({
            groups: ['RESIDENTIAL'],
            session: session.id,
        });
    ...
    _getCustomeProxyUrl(session, proxy) {
        return Apify.getApifyProxyUrl({
            groups: [proxy],
            session: session.id,
        });
